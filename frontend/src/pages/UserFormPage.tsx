    ...
    <>
      ...
      <div
        className={`flex w-full items-center justify-center p-8 ${
          isPublicSignUp ? "min-h-screen bg-slate-50 font-sans" : ""
        }`}
      >
        <div className="w-full max-w-md">
          <h1 className="text-4xl font-bold text-slate-800">{pageTitle}</h1>
          <p className="mt-3 mb-8 text-slate-500">{pageSubtitle}</p>
          <form ref={formRef} onSubmit={handleSubmit} className="space-y-6">
            ...
            {user?.role === "admin" && !isEditMode && (
              <div className="pt-2">
                <LabelPrimitive.Root className="block text-sm font-medium text-slate-700 mb-2">
                  Role
                </LabelPrimitive.Root>
                <Select.Root
                ...
                >
                  <Select.Trigger className="relative w-full cursor-default rounded-lg border border-slate-300 bg-white py-2.5 pl-3 pr-10 text-left shadow-sm focus:outline-none focus:ring-2 focus:ring-brand-500 focus:border-brand-500">
                    <Select.Value />
                  ...
                  <Select.Portal>
                    <Select.Content
                      position="popper"
                      sideOffset={5}
                      className="z-10 mt-1 w-[var(--radix-select-trigger-width)] rounded-md bg-white shadow-lg ring-1 ring-black ring-opacity-5"
                    >
                      <Select.Viewport className="p-1">
                        {roles.map((role) => (
                          <Select.Item
                            key={role.value}
                            value={role.value}
                            className="relative cursor-default select-none rounded-md py-2 pl-10 pr-4 text-sm outline-none data-[highlighted]:bg-brand-100 data-[highlighted]:text-brand-900"
                          >
